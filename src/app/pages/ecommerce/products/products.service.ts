...
export class AdvancedService {
  ...
  constructor(private pipe: DecimalPipe, public ApiService: restApiService, private http: HttpClient) {
    this._search$.pipe(
      ...
      this._allproduct$.next(result.allproduct);
  ...
  private _search(): Observable<SearchResult> {
    ...
    const { sortColumn, sortDirection, pageSize, page, searchTerm, ProductFilter, productRate, productPrice,productStatus } = this._state;
    ...
    // 3. filter
    if(ProductFilter){
      countries = countries.filter(country => matches(country, ProductFilter, this.pipe));
    }
    ...
    // 6.Price &  rate Filter
    if (productPrice) {
      countries = countries.filter(country => country.price >= Object.values(productPrice)[0] && country.price <= Object.values(productPrice)[1]);
    }
    else {
      countries = countries;
    }
    ...
    // 6 Status Filter
    if (productStatus) {
      countries = countries.filter(country => country.status == productStatus);
    }
    else {
      countries = countries;
    }
